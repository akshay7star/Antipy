...
5. **Stage and Smart Commit**
   Stage all changes. **CRITICAL:** Ensure that all AI-related data (such as `.agents`, `.project_history`, and hidden agent scripts) are ignored via `.gitignore` and NEVER committed to Git. The remote repository must look exactly like a standard human developer's project. Write a concise, descriptive commit message using the Conventional Commits format, referencing the version if bumped.
   ```bash
   git add .
...
6. **Synchronize Project History**
   Copy the latest `task.md` and `implementation_plan.md` from the hidden agent directory to the local project `.project_history` folder to ensure transparency and history syncing locally. Do not commit `.project_history` to git.
